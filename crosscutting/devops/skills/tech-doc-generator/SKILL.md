...
description: Generate comprehensive technical documentation for microservices and repositories by analyzing source code. Use when the user asks to generate, create, or write technical documentation for a microservice, service, repository, or codebase. Produces enterprise-grade docs including service overview, architecture diagrams, API reference, business rules, process flows, sequence diagrams, external integrations, resilience patterns, data models, error handling, security, configuration, and deployment notes. All documentation is strictly derived from source code — nothing is fabricated. Uses Mermaid diagrams for visual representations. Target audience is developers and architects.
---

# Technical Documentation Generator

Generate enterprise-grade technical documentation for microservices and repositories by systematically analyzing source code. Every statement in the output is traceable to the codebase — nothing is assumed or fabricated.
...
The skill produces a `docs/` folder with the following files. **Only generate files for which relevant code exists.** If a section has no backing code (e.g., no resilience patterns found), skip that file and note the omission in the index.
...
1. **Entry point & bootstrap** — Read the main class/file to understand how the app starts, what modules/beans are registered
...
See [references/mermaid-guide.md](references/mermaid-guide.md) for complete syntax and patterns.

### Phase 5: Document Assembly

Write each documentation file following the templates in [references/doc-templates.md](references/doc-templates.md).
...
6. **Omit empty sections** — if no relevant code exists for a document, do not generate it; list it as omitted in the index
...
- **File references**: Always include relative file path when citing source code: `(see src/service/OrderService.java:45)`
...
| File | Purpose |
|------|---------|
| [references/doc-templates.md](references/doc-templates.md) | Markdown templates for each documentation file |
| [references/mermaid-guide.md](references/mermaid-guide.md) | Mermaid syntax reference for diagrams used in documentation |
